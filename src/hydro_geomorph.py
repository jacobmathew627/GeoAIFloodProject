...
def calculate_geomorphic_factors():
    ...
    with rasterio.open(dem_path) as dem_src:
            ...
            for y in range(0, height, BLOCK_SIZE):
                for x in range(0, width, BLOCK_SIZE):
                    ...
                    if np.any(mask):
                        ...
                        sca_val = (
                            (np.exp(flow[mask]) * CELL_SIZE)
                            if np.mean(flow[mask]) < 15
                            else (flow[mask] * CELL_SIZE)
                        )
        ...
        with rasterio.open(lulc_path) as lulc_src, rasterio.open(flow_path) as flow_src:
            ...
            if np.any(ds_drainage):
                ...
                h_fit, w_fit = min(dem.shape[0], hand_up.shape[0]), min(
                    dem.shape[1], hand_up.shape[1]
                )
                hand[:h_fit, :w_fit] = hand_up[:h_fit, :w_fit]
                del ds_dem, ds_drainage, indices, ds_hand, hand_up
                gc.collect()
            else:
                hand = np.full(dem.shape, nodata, dtype=np.float32)
            ...
            with rasterio.open(
                os.path.join(OUTPUT_DIR, "DistWater_aligned.tif"), "w", **profile
            ) as dst:
                dist_water[dem == nodata] = nodata
                dst.write(dist_water, 1)

            with rasterio.open(
                os.path.join(OUTPUT_DIR, "DistUrban_aligned.tif"), "w", **profile
            ) as dst:
            ...
            del lulc, dem, water_mask, urban_mask, dist_water, dist_urban, hand
            gc.collect()
